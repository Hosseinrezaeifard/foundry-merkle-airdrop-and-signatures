...
# Compile and deploy ByteBuckToken contract
echo "Creating zkSync local node..."
npx zksync-cli dev start
echo "Deploying token contract..."
TOKEN_ADDRESS=$(forge create src/ByteBuckToken.sol:ByteBuckToken --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --legacy --zksync | awk '/Deployed to:/ {print $3}' )
...
# Split signature 
SIGN_OUTPUT=$(forge script script/ZkSplitSignature.s.sol:ZkSplitSignature)
